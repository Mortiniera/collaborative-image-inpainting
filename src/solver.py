...
class Solver(object):
    ...
    def train(self, fixed=False, fixed_z=None):
        ...
        while self.iter_time < self.flags.iters:
            ...
            self.sample(self.iter_time, fixed, fixed_z)
    ...
    def sample(self, iter_time, fixed=False, fixed_z=None):
        if (np.mod(iter_time, self.flags.sample_freq) == 0 and self.iter_time != 0):
            imgs = self.model.sample_imgs(fixed, fixed_z)
